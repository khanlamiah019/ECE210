...
clc; clear; close all;
		...
		% <-.01> (switchsign) mostly works here (and nicely done), but
		% technically has an error: calling `switchsign([-1 2 1 0 1
		% -3])` (the example i gave) returns `[0 1 0 1 1 1]` rather
		% than `[0 1 0 0 0 1]`. not that that really matters.


%% my sinc is broken 
% (4) (-7,7) range is used for x mostly because I thought it looked better.
%x = linspace(-7, 7);
		% <-.01> i might recommend increasing the number of points on
		% the curve, just so you get a cleaner graph:
		x = linspace(-7, 7, 999);
y = sinc(x);
...
% perhaps the texture made it seem brighter. lastly,line style created the mesh grid.  
		...
		% <+.03> that is an extremely cool surface. i will have to
		% remember this code for when i need to do something
		% interesting.
